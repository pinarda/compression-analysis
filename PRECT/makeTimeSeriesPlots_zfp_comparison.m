...
function[] = makeTimeSeriesPlots(MN_lat, MN_lon, Ocean_lat, Ocean_lon, variable, wd, save_dir)
    ...
    MN_compress_zfp_beta = zeros(N, 5);
    MN_compress_zfp = zeros(N, 5);
    Ocean_compress_zfp_beta = zeros(N, 5);
    Ocean_compress_zfp = zeros(N, 5);
    for tol_j = 1:4
        tol_ind_sz = tol_list_sz{tol_j};
        tol_ind_zfp = tol_list_zfp{tol_j};
        MN_compress_zfp_beta(:,tol_j) = ncread([wd, 'zfp_beta/prect/zfpATOL', tol_ind_zfp, ...
        ...
        Ocean_compress_zfp_beta(:,tol_j) = ncread([wd, 'zfp_beta/prect/zfpATOL', tol_ind_zfp, ...
            '/b.e11.B20TRC5CNBDRD.f09_g16.030.cam.h1.PRECT.19200101-20051231.nc'], ...
            variable, [145+64 84 1], [1 1 Inf]);
    ...
    title(['Lat ', num2str(round(MN_lat,2)), ' Lon ', num2str(MN_lon), ', zfp beta'])
    xlabel('Time')
    ylabel('PRECT')

    subplot(2,2,1)
    plot(1:50, MN_orig(1:50), '+-k', 'MarkerSize', 10, 'LineWidth', 2, 'MarkerIndices', 1:5:50); hold on
    title(['Lat ', num2str(round(MN_lat,2)), ' Lon ', num2str(MN_lon), ', zfp 0.5.3'])
    xlabel('Time')
    ylabel('PRECT')

    subplot(2,2,4)
    plot(1:50, Ocean_orig(1:50), '+-k', 'MarkerSize', 10, 'LineWidth', 2, 'MarkerIndices', 1:5:50); hold on
    title(['Lat ', num2str(round(Ocean_lat,2)), ' Lon ', num2str(Ocean_lon), ', zfp beta'])
    xlabel('Time')
    ylabel('PRECT')

    subplot(2,2,3)
    plot(1:50, Ocean_orig(1:50), '+-k', 'MarkerSize', 10, 'LineWidth', 2, 'MarkerIndices', 1:5:50); hold on
    title(['Lat ', num2str(round(Ocean_lat,2)), ' Lon ', num2str(Ocean_lon), ', zfp 0.5.3'])
    xlabel('Time')
    ylabel('PRECT')
    ...
    for k = 1:4
        subplot(2,2,2)
        plot(1:50, MN_compress_zfp_beta(1:50,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:5:50);

        subplot(2,2,1)
        plot(1:50, MN_compress_zfp(1:50,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:5:50);

        subplot(2,2,4)
        plot(1:50, Ocean_compress_zfp_beta(1:50,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:5:50);
