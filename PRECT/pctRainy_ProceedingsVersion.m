%Plots the probability of rain by day of year, globally and across years

clear *

addpath('/glade/u/home/apoppick/MATLABPackages/b2r');
addpath('/glade/u/home/apoppick/MATLABPackages/subaxis');
addpath('/glade/u/home/apoppick/MATLABPackages/freezeColors');
addpath('/glade/u/home/apoppick/MATLABPackages/cm_and_cb_utilities');

wd = '/glade/p/cisl/iowa/abaker_carleton/';

variable = 'PRECT';

alg_prefix_list = {'szAOn','zfpATOL'};

tol_list_zfp = {'1e-2', '1e-5', '1e-8', '1e-11', '0'};
%Not doing 0.0001 and 1e-05 because zscores are all zero
tol_list_sz = {'0.01', '1e-05', '1e-08', '1e-11'};

%% read in lat / lon and original data
model_lat = ncread([wd, lower(variable), '/orig/', ...
    'b.e11.B20TRC5CNBDRD.f09_g16.030.cam.h1.', ...
    variable, '.19200101-20051231.nc'], ...
    'lat');
model_lon = ncread([wd, lower(variable), '/orig/', ...
    'b.e11.B20TRC5CNBDRD.f09_g16.030.cam.h1.', ...
    variable, '.19200101-20051231.nc'], ...
    'lon');
model_lon(model_lon > 180) = model_lon(model_lon > 180) - 360;
model_lon = [model_lon(146:288); model_lon(1:145)]; %reorganize longitudes

orig_data = ncread([wd, lower(variable), '/orig/', ...
            'b.e11.B20TRC5CNBDRD.f09_g16.030.cam.h1.', ...
            variable, '.19200101-20051231.nc'], ...
            variable);
orig_data = orig_data([146:288, 1:145], :, :); %reorganize longitudes

nLon = size(model_lon, 1);
nLat =  size(model_lat, 1);
N =  size(orig_data, 3);

%% Calculate Statistics for Original Data

rainDays_orig = (orig_data > 0);
pctRainy_orig = squeeze(mean(mean(rainDays_orig,1),2));
pctRainy_sz = zeros(N, length(tol_list_sz));
pctRainy_zfp = zeros(N, length(tol_list_zfp));
for alg_i = 1:2
    alg_prefix = alg_prefix_list{alg_i};
    if strcmp(alg_prefix, 'zfpATOL') 
        tol_list = tol_list_zfp;
        compress_alg = 'zfp';
    else
        tol_list = tol_list_sz;
        compress_alg = 'sz';
    end
    for tol_j = 1:length(tol_list) %non-lossless tolerances
        tol = tol_list{tol_j};
        tic
        %% Read in Compressed Data
        compress_data = ncread([wd, lower(variable), '/', alg_prefix, tol_list{tol_j}, '/', ...
            'b.e11.B20TRC5CNBDRD.f09_g16.030.cam.h1.', ...
            variable, '.19200101-20051231.nc'], ...
            variable);
        compress_data = compress_data([146:288, 1:145], :, :); %reorganize longitudes
        
        if strcmp(compress_alg, 'sz')
            pctRainy_sz(:,tol_j) = squeeze(mean(mean(compress_data > 0,1),2));
        else
            pctRainy_zfp(:,tol_j) = squeeze(mean(mean(compress_data > 0,1),2));
        end
    end
end

seaspctRainy_orig = squeeze(mean(reshape(pctRainy_orig, 365, []),2));
seaspctRainy_zfp = squeeze(mean(reshape(pctRainy_zfp, 365, N/365, 5),2));
seaspctRainy_sz = squeeze(mean(reshape(pctRainy_sz, 365, N/365, 4),2));

seaspctRainy_origAdj = (squeeze(sum(reshape(pctRainy_orig, 365, []),2)) + 1)./(N/365+2);
seaspctRainy_zfpAdj = (squeeze(sum(reshape(pctRainy_zfp, 365, N/365, 5),2)) + 1)./(N/365+2);
seaspctRainy_szAdj = (squeeze(sum(reshape(pctRainy_sz, 365, N/365, 4),2)) + 1)./(N/365+2);

logOR_zfp = log10( (seaspctRainy_zfpAdj./(1-seaspctRainy_zfpAdj)) ./ ...
                    (seaspctRainy_origAdj./(1-seaspctRainy_origAdj)) );
logOR_sz = log10( (seaspctRainy_szAdj./(1-seaspctRainy_szAdj)) ./ ...
                    (seaspctRainy_origAdj./(1-seaspctRainy_origAdj)) );
                

Markers = {'o', '*', 'x', 'square', '^'};
Colors = {'r', 'b', 'g', 'm', 'c'};

subplot(1,4,1), plot(seaspctRainy_orig, '+-k', 'MarkerSize', 10, 'LineWidth', 2, 'MarkerIndices', 1:30:365); hold on
for k = 1:4
    subplot(1,4,1), plot(seaspctRainy_sz(:,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365)
end
xlim([1 365])
ylim([0 1])
ylabel("% rainy days")
xlabel("day")
title('sz')

subplot(1,4,2), plot(logOR_sz(:,1), strcat( Markers{1}, '-', Colors{1}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365); hold on
for k = 2:4
    subplot(1,4,2), plot(logOR_sz(:,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365)
end
xlim([1 365])
ylim([-4 0.5])
ylabel("Log10(Odds Ratio)")
xlabel("day")
title('sz')

subplot(1,4,3), plot(seaspctRainy_orig, '+-k', 'MarkerSize', 10, 'LineWidth', 2, 'MarkerIndices', 1:30:365); hold on
for k = 1:5
    subplot(1,4,3), plot(seaspctRainy_zfp(:,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365)
end
xlim([1 365])
ylim([0 1])
ylabel("% rainy days")
xlabel("day")
title('zfp')
subplot(1,4,3), legend(['Original', tol_list_zfp], 'Location', 'West')

subplot(1,4,4), plot(logOR_zfp(:,1), strcat( Markers{1}, '-', Colors{1}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365); hold on
for k = 2:5
    subplot(1,4,4), plot(logOR_zfp(:,k), strcat( Markers{k}, '-', Colors{k}), 'MarkerSize', 7, 'MarkerIndices', 1:30:365)
end
xlim([1 365])
ylim([-4 0.5])
ylabel("Log10(Odds Ratio)")
xlabel("day")
title('zfp')


set(gcf,'Units', 'inches', 'Position', [0 0 12 4], 'PaperUnits','inches','PaperPosition', [0 0 12 4])


save_path = '/glade/work/apoppick/compress_plots/PRECT_odds/pctrainy_annual_AllisonPlot.png';
print(save_path, '-dpng')