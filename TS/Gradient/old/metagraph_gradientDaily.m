% This code plots EW gradient fields.
...
grad_lon = zeros(nLon - 1, nLat, length(orig_jan));
grad_lon(:, :, :) =  orig_jan(2:end, :, :) - orig_jan(1:end - 1, :, :);
mean_grad_lon = mean(grad_lon, 3);
...
framem; tightmap;
load coast;
pcolorm([-90 90],[-180,180], mean_grad_lon');
plotm(lat,long, 'k');
cbar_max_orig = prctile(mean_grad_lon(:), 95);
cbar_min_orig = prctile(mean_grad_lon(:), 5);
colormap(flipud(b2r(cbar_min_orig, cbar_max_orig)));
C2 = colorbar('southoutside', 'FontSize', 5);
text(-2.5, 2, 'Original');
text(-7,3.5, ['Mean Gradient of ', char(variable), ' Across Longitudes'], 'FontWeight', 'bold', 'FontSize', 10);
...
% Compressed Data
        ...
        grad_lon =  compress_jan(2:end, :, :) - compress_jan(1:end - 1, :, :);
        mean_grad_lon = mean(grad_lon, 3);
        ...
        pcolorm([-90 90],[-180,180], mean_grad_lon');
        plotm(lat,long, 'k');
        cbar_max(j,i) = prctile(abs(mean_grad_lon(:)), 95);
...
save_path = ['/glade/u/home/feldmann/GradientLonTS.png'];
