% Creates a 4x2 plot of the day with the largest mean absolute error for TS
...
% bottom.
function [] = spatial_plot_mae_max_day_new(diff_datas, alg_prefix_list, tol_list_sz, tol_list_zfp, save_dir, nLat, nLon, variable)
    ...
    for i=1:length(alg_prefix_list)
        ...
        for j=1:length(tol_list)
            ...
            [val, idx] = max(mae_location, [], 2);
            mae_max_location = reshape(idx, nLon, nLat);
            ...
            framem; tightmap;
            load coast;
            pcolorm([-90 90],[-180,180], mae_max_location');
            plotm(lat,long, 'k');
            ...
            if (strcmp(alg_prefix, string(alg_prefix_list(2))) && strcmp(tol, string(tol_list(1))))
                text(-8,3, ['Day with maximum MAE for ', char(variable)], 'FontWeight', 'bold', 'FontSize', 10)
            end
        end
    end
    ...
    if strcmp('1.0', string(tol_list_zfp(1)))
        save_path = [save_dir, 'maxmae', char(variable), '.png'];
    else
        save_path = [save_dir, 'maxmae', char(variable), '_tight_tolerance', '.png'];
    end
