...
function[] = contrastVarianceEWTS_zfp_comparison(orig_data, compressed_datas, alg_prefix_list, tol_list_sz, tol_list_zfp, nLat, nLon, obs, save_dir, days)

    tol_list_zfp = {'1.0', '0.5', '1e-1'};
    %%Load Land Fractions
...
%                     ' Sea:', num2str(seaVarianceOrig, 4), ')'], 'FontSize', 7);
    ...
    % Compressed Data
            ...
            ax(i, j) = subaxis(length(alg_prefix_list),length(tol_list_zfp),j, i,'Spacing', 0, 'SpacingHoriz' , 0 , 'SpacingVert' , 0 , ... 
                'Padding', 0, 'PaddingRight', 0, 'PaddingLeft', .02 , 'PaddingTop', 0.0 , 'PaddingBottom', 0 , ... 
                'Margin', 0 , 'MarginRight', 0.03, 'MarginLeft', 0.1 , 'MarginTop', 0.06 , 'MarginBottom', 0.04, 'HoldAxis', 1);
            axesm('robinson');
            set(gca, 'LooseInset', get(gca,'TightInset'));
            framem; tightmap;
            load coast;
            ...
            colormap(ax(i,j), flipud(w2k_new(-cbar_max(j+i*5), cbar_max(j+i*5))));
            ...
            text(-2.5,1.6, ['Ratio:', num2str(varRatio, 4), ...
                    ' (Land:', num2str(landRatio, 4),  ...
                    ' Sea:', num2str(seaRatio, 4), ')'], 'FontSize', 10);
            %colormap(flipud('gray'))
    ...
    set(gcf,'Units', 'inches', 'Position', [0 0 9 5], 'PaperUnits','inches','PaperPosition', [0 0 9 5])
    if strcmp('1.0', string(tol_list_zfp(1)))
        save_path = [save_dir, 'ContrastVarianceEWTS_new.png'];
    else
        save_path = [save_dir, 'ContrastVarianceEWTS_new', '_tight_tolerance', '.png'];
    end
