...
function[] = contrastVarianceNSTS_new(orig_data, compressed_datas, alg_prefix_list, tol_list_sz, tol_list_zfp, nLat, nLon, obs, save_dir, days, mo_length)
    ...
    diff_sq = zeros(nLon, nLat-1, length(janObs));
    log_con_var = zeros(nLon, nLat-1);
    ...
    set(gcf,'Units', 'inches', 'Position', [0 0 5 8], 'PaperUnits','inches','PaperPosition', [0 0 5 8])
    if strcmp('1.0', string(tol_list_zfp(1)))
        save_path = [save_dir, 'ContrastVarianceNSTS_new', num2str(days),'.png'];
    else
        save_path = [save_dir, 'ContrastVarianceNSTS_new', '_tight_tolerance', char(days), '.png'];
    end
